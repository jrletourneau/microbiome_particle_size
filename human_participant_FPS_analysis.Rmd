...
moisture_summary <- moisture %>%
  group_by(cluster) %>%
  summarize(mean = mean(moisture), se = sd(moisture)/sqrt(length(moisture)))
...
# TukeyHSD signif for 1-2, 1-3, 2-4, 3-4 --> a, b, b, a, ab
...
moisture_plot <- ggplot(moisture, aes(x = cluster, y = moisture)) +
  ...
  geom_errorbar(data = moisture_summary, aes(x=cluster, ymin=mean-se, ymax=mean+se),
                width=0.25, inherit.aes = F) + 
  annotate(geom = "text", x = 1:5, y = c(90, 87, 90, 85, 88),
           label = c("a", "b", "b", "a", "ab")) +
...
ord_plot_trnL <- plot_ordination(trnL, ord_trnL, color="median_FPS", title="Bray NMDS") +
...
# adonis2(formula = bray_trnL ~ median_FPS, data = samdf_trnL,
#         strata=samdf_trnL$participant, permutations=9999, method = "bray")
...
for(r in 1:nrow(bray_select)) {
  ...
  bray_select$dist[r] <- bray[rownames(bray_select)[r], relevant_first_sample]
...
for(r in 1:nrow(bray_select2)) {
  ...
  # On bray dataframe, get distance between current sample and relevant first sample
  bray_select2$dist[r] <- bray[rownames(bray_select2)[r], relevant_first_sample]
...
for(r in 1:nrow(bray_change)) {
  ...
  bray_change$dist[r] <- bray[rownames(bray_change)[r], previous_sample]
...
# Figure 3 - SCFA and diet
fig3_patch <- plot_grid(scfa_total_plot, scfa_indiv_plot,
                        ncol = 1, rel_heights = c(2, 3),
                        labels = c("A", "B"), label_fontface = "plain")

#ggsave("plots/fig3.png", fig3_patch, height = 8.4, width = 6)
...
fig4_Lcorner <- plot_grid(median_plot +
                          theme(legend.position = "none"),
...
fig4_Rcorner <- plot_grid(ord_plot2 +
                          theme(legend.position = "none"),
                         get_legend(ord_plot2),
                         ncol = 1, rel_heights = c(3, 1.3),
                         labels = c("D", ""), label_fontface = "plain")

fig4_bottom <- plot_grid(fig4_Lcorner, fig4_Rcorner, 
                         nrow = 1, rel_widths = c(3, 2.3),
...
#ggsave("plots/fig4.png", fig4_patch, height = 8, width = 7)
...
figS7_patch <- plot_grid(figS7_top, figS7_mid,
                         chew_scfa_plot + theme(legend.position = "none"),
                         chew_scfa_prop_plot + theme(legend.position = "none"),
                         nrow = 4,
...
figS11_patch <- plot_grid(macro_corr_plot, diet_macro_pca_plot,
                         ncol = 1, rel_heights = c(2.5, 1),
                         labels = c("A", "B"), label_fontface = "plain")

#ggsave("plots/figS11.png", figS11_patch, height = 9, width = 6.2)
